...
// Sign-in matches on username, so a row without one cannot log in at all.
const username = process.env.TEST_ADMIN_USERNAME ?? 'techcadd'
...
await connection.query(
  ...
  [randomUUID(), email, username, await argon2.hash(password, { type: argon2.argon2id, memoryCost: 19456, timeCost: 2 })],
)
