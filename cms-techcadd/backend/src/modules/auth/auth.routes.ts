...
import { config, loginAttemptLimit } from '../../config.js'
...
const cookieOptions = {
  ...
  // Set explicitly rather than inferred from NODE_ENV — see config.ts.
...
/*
  Surrounding whitespace is trimmed off every password, wherever one is
  accepted.

  These are machine-generated and copy-pasted: a one-time password picked up
  with a leading space is invisible on screen, and the sign-in error cannot say
  which half is wrong without telling an attacker the same thing. That trap
  cost a real sign-in.

  Trimmed on the way in *and* on the way out — at login, at reset, at change,
  and when an account is created — so the rule is simply "a password has no
  whitespace at either end" rather than a leniency at one door that quietly
  changes what was stored at another.
*/
