...
/**
 * Two roles: an editor handles content, an admin can also reach settings,
 * redirects, enquiries and accounts. See 024_editor_role.sql.
 */
 ...
 *
 * Deliberately not "username or email": the form asks for one thing and says
 * so, and accepting a second identifier the label never mentions is the kind of
 * undocumented behaviour that later gets mistaken for a bug. An account whose
 * username is NULL cannot sign in — see 018_user_username.sql.
 */
export async function login(
  ...
  const user = await queryOne<UserRow>(
    'SELECT id, name, email, username, password_hash, role, active FROM users WHERE username = ? LIMIT 1',
  ...
  if (!user || !ok) throw unauthorised('That username and password combination is not recognised.')
  ...
  /*
    Sweep out sessions that have already expired.

    `purgeExpired` was written to be called "from a scheduled job", and there
    is no scheduled job — so nothing ever called it and the table only grew.
    Signing in is the natural moment: it is exactly when a new row appears, it
    is infrequent, and the delete is bounded by the index on expires_at.

    Not awaited, so a slow sweep can never delay somebody signing in, and a
    failed one can never stop them.
  */
  void purgeExpired().catch((error: unknown) => {
    console.error('[auth] could not purge expired sessions:', error)
  })
