...
<div style="font-family:system-ui,-apple-system,Segoe UI,sans-serif;color:#0f172a;line-height:1.6">
  ...
  <p style="color:#64748b;font-size:12px;margin-top:24px">
    ${escapeHtml(config.CMS_NAME)} · this message was sent automatically, please do not reply.
  </p>
</div>`
...
export function passwordResetEmail(to: string, token: string): Mail {
  ...
  const text = [
    `Someone asked to reset the password for this ${config.CMS_NAME} account.`,
    '',
  ...
  return {
    to,
    subject: `Reset your ${config.CMS_NAME} password`,
    text,
    html: shell(
      'Reset your password',
      `<p>Someone asked to reset the password for this ${escapeHtml(config.CMS_NAME)} account.</p>
