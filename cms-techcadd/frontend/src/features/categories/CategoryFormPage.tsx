    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 pb-24">
      ...
      <Card flush className="max-w-3xl">
        ...
        <CardBody className="grid gap-5 sm:grid-cols-2">
          ...
          {/*
            No baseUrl, because a category has no page on this site.

            It used to show "techcadd.com/courses/category/", which is a route
            that does not exist here — a category is the heading its courses are
            filed under in the menus, not an address. The slug still matters
            (it is how the site matches a category to its accent colour and
            ordering), so the field stays; the URL that was never real does not.
          */}
          <FormField
            label="URL slug"
            required
            description="Not a web address — a category is a heading in the menus. The slug is how it is matched up, so keep it stable once courses are filed under it."
          ...
          >
            <Controller
              ...
              name="slug"
              render={({ field }) => (
                <SlugInput value={field.value} onChange={field.onChange} source={name} />
              )}
            />
          </FormField>
